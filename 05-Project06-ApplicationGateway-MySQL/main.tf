...
resource "azurerm_linux_virtual_machine_scale_set" "app1_web_vmss" {
  ...
  custom_data = base64encode(templatefile("${path.module}/scripts/ubuntu-webvm-script.sh", {
    ...
    db_name     = azurerm_postgresql_flexible_server_database.webappdb.name
...
resource "azurerm_private_dns_zone" "postgres_dns" {
  name                = "${local.resource_group_prefix}-postgres.private.postgres.database.azure.com"
  resource_group_name = data.azurerm_resource_group.rg.name
}

# Link Private DNS Zone to Virtual Network
resource "azurerm_private_dns_zone_virtual_network_link" "postgres_dns_vnet_link" {
  name                = "${local.resource_group_prefix}-postgres-dns-link"
...
resource "azurerm_postgresql_flexible_server" "postgres_server" {
  name                   = "${local.resource_group_prefix}-pg-${random_string.default.id}"
  resource_group_name    = data.azurerm_resource_group.rg.name
  location               = var.resource_location
  administrator_login    = var.postgres_db_username
  administrator_password = var.postgres_db_password
...
resource "azurerm_postgresql_flexible_server_database" "webappdb" {
